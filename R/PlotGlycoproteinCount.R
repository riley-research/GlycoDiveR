...
PlotGlycoproteinCount <- function(input, grouping = "condition", whichAlias = NULL,
  ...
  if(grouping == "technicalReps"){
    tempdf <- input$PSMTable %>%
      ...
      dplyr::distinct(.data$Run, .data$UniprotIDs, .keep_all = TRUE) %>%
    ...
    p <- ggplot2::ggplot(tempdf, ggplot2::aes(x=.data$Alias, y = .data$PSMCount, fill = .data$Condition)) +
      ...
      ggplot2::labs(x = "", y = "Unique glycoproteins (count)") +
